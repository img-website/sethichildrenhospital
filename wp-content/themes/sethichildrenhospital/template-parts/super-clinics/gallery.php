<?php
/**
 * Super Speciality Clinics – Gallery
 */

$images = sch_get_field('sc_gallery_images');
?>
...
<section class="overflow-hidden py-12 md:py-16 bg-white">
    <div class="max-w-7xl mx-auto px-4">
        <div class="text-center mb-8" data-aos="fade-up" data-aos-duration="700">
            ...
            <h2 class="text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold font-anek-gujarati text-gray-900 mt-2">State-of-the-Art <span class="text-secondary">Infrastructure</span></h2>
        </div>
            ...
            <div class="swiper gallery-swiper rounded-2xl overflow-hidden shadow-2xl" data-slide-count="<?php echo esc_attr(count($images)); ?>">
                <div class="swiper-wrapper">
                    <?php foreach ($images as $img) :
                        $url = $img['image'] ?? '';
                        if (!$url) continue;
                        $alt = $img['alt'] ?? 'Super Speciality Clinic';
                    ?>
                    <div class="swiper-slide">
                        <img src="<?php echo esc_url($url); ?>" alt="<?php echo esc_attr($alt); ?>" class="w-full h-[300px] md:h-[400px] lg:h-[500px] object-cover">
                    </div>
                    <?php endforeach; ?>
                </div>
